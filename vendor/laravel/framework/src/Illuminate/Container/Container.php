...
class Container implements ArrayAccess {
	...
	{
		if ( ! $this->bound($abstract)) //NOTE: 判断是否还未绑定过
		{
	...
	public function extend($abstract, Closure $closure)
	{
		...
		{
            //NOTE: 如果已绑定过，通过closure“装饰”已绑定的实例
	 ...
	 *
	 * @param  string  $abstract
	 * @param  mixed   $instance NOTE: 任何类型
	 * @return void
	 */
	...
	protected function rebound($abstract)
	{
		...
		{
			call_user_func($callback, $this, $instance); //NOTE: fire rebound callbacks
		}
	}
	...
	public function make($abstract, $parameters = array())
	{
		...
		{
			$object = $this->make($concrete, $parameters); //NOTE: nested
		}
		...
		{
			$this->instances[$abstract] = $object; //NOTE: instances保存所有共享的对象
		}

		$this->fireResolvingCallbacks($abstract, $object);

		$this->resolved[$abstract] = true; //NOTE: resolved保存所有make过的对象
	...
	protected function getDependencies($parameters, array $primitives = array())
	{
		...
		foreach ($parameters as $parameter)
		{
			...
			else //NOTE: 构造参数含有对象, 自动make所依赖的类
			{
	...
	{
		if ($parameter->isDefaultValueAvailable()) //NOTE: 默认参数
		{
